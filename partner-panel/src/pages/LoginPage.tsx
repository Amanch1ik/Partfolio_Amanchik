...
export const LoginPage = () => {
  ...
  const onFinish = async (values: { username: string; password: string }) => {
    debounceClick(async () => {
      ...
      // DEV MODE shortcut: не бьем в API, сразу пускаем
      if (devModeEnabled) {
        const devUser = {
          id: 'dev',
        ...
        localStorage.setItem('partner_token', 'dev-token');
        localStorage.setItem('partner_user', JSON.stringify(devUser));
        setUser(devUser);
        useAuthStore.getState().setLoading(false);
        message.success('DEV вход выполнен');
        setLoading(false);
      ...
      try {
        ...
        if (response && response.access_token) {
          ...
          const userData = rawResponse.partner || rawResponse.user || {
