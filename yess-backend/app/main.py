...
# Дополнительный слой CORS на случай, если стандартный middleware не отработает
@app.middleware("http")
async def add_cors_headers(request: Request, call_next):
    origin = request.headers.get("origin")
    # Пускаем любые локальные origin, чтобы dev-панели не упирались в CORS
    if origin and ("localhost" in origin or "127.0.0.1" in origin):
        if request.method == "OPTIONS":
            # Быстрый ответ для preflight
    ...
    if origin and ("localhost" in origin or "127.0.0.1" in origin):
        ...
        response.headers.setdefault("Access-Control-Allow-Methods", "GET, POST, PUT, DELETE, OPTIONS, PATCH, HEAD")
