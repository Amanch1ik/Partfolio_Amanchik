...
// Support both legacy VITE_API_URL and new VITE_API_BASE_URL
const API_BASE_URL =
...
const adminApi = {
  ...
  async getCurrentAdmin(): Promise<ApiResponse<AdminUser>> {
    // В продакшене ходим в API, в dev режиме пусть фронт не вызывает
  ...
  // Assets upload
  async uploadAsset(folder: string, file: File): Promise<ApiResponse<any>> {
    const formData = new FormData();
    formData.append('folder', folder);
    formData.append('file', file);

    const response = await apiClient.post('/upload/admin/assets', formData, {
