...
import { Layout, Menu, Avatar, Dropdown, Badge, Input, message, Modal, Form, Upload, Select } from 'antd';
...
export const MainLayout = ({ children }: MainLayoutProps) => {
          ...
          label: (
            <div style={{ maxWidth: 300, padding: '8px 0' }}>
              <p style={{ margin: 0, fontWeight: 500 }}>
                {notif.title || t('notifications.notification', 'Уведомление')}
              </p>
  ...
  const handleLanguageChange = (lang: string) => {
    ...
    const langChanged = t('language.changed', 'Язык изменён');
    message.success(`${langChanged} ${langName}`);
  ...
  const menuItems = [
    ...
    },
    {
      key: '/assets',
      icon: <UploadOutlined />,
      label: <Link to="/assets">{t('nav.assets', 'Ресурсы')}</Link>,
    },
    {
      key: '/audit',
      icon: <AuditOutlined />,
  ...
  const updateProfileMutation = useMutation({
    mutationFn: async (values: any) => {
      ...
      // В dev/стаб режиме сохраняем локально и возвращаем мок-ответ без запроса
      return Promise.resolve({ data: { admin: updateData } });
    },
    onSuccess: (response) => {
