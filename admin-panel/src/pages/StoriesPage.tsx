...
export const StoriesPage = () => {
  ...
  const toggleMutation = useMutation({
    mutationFn: (id: number) => storiesApi.toggle(id),
    onSuccess: () => {
      message.success('Статус сториса обновлен');
      queryClient.invalidateQueries({ queryKey: ['stories'] });
    },
    onError: (error: any) => {
  ...
  const handleEdit = (story: Story) => {
    ...
    form.setFieldsValue({
      ...
      image: story.image_url
        ? ({ mode: 'url', url: story.image_url, fileList: [] } as MediaInputValue)
        : undefined,
    });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = () => {
    form.validateFields().then((values) => {
      const process = async () => {
        const imageUrl = await resolveMedia(values.image);
        const payload = {
          ...values,
    ...
    <div style={{ padding: '24px' }}>
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="image" label="Изображение" valuePropName="value" rules={[{ required: true }]}>
            <MediaInput placeholderUrl="https://example.com/image.jpg" accept="image/*" />
          </Form.Item>
