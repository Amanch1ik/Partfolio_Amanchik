...
export const PromotionsPage = () => {
  ...
  const handleEdit = (promotion: Promotion) => {
    ...
    form.setFieldsValue({
      ...
      banner: promotion.promo_image_url
        ? ({ mode: 'url', url: promotion.promo_image_url, fileList: [] } as MediaInputValue)
        : undefined,
    });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = () => {
    form.validateFields().then((values) => {
      const process = async () => {
        const bannerUrl = await resolveMedia(values.banner);
        const payload = {
          ...values,
          start_date: values.dates[0].toISOString(),
          end_date: values.dates[1].toISOString(),
          promo_image_url: bannerUrl ?? values.promo_image_url,
        };
    ...
    <div className="fade-in">
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 24 }}>
          ...
          <Form.Item name="banner" label={t('promotions.banner', 'Баннер/картинка')} valuePropName="value">
            <MediaInput placeholderUrl="https://example.com/banner.png" accept="image/*" />
          </Form.Item>
        </Form>
      </Modal>
    </div>
