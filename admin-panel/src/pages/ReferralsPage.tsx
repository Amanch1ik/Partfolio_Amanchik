...
import { Table, Card, Button, Space, Avatar, Dropdown, message, Empty } from 'antd';
...
export const ReferralsPage = () => {
  ...
  const { data: referralsResponse, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // API может возвращать массив или обернутый объект
      const items = Array.isArray(res?.data) ? res.data : res?.data?.items;
      const total = res?.data?.total || items?.length || 0;
      return { items: items || [], total };
    },
    retry: 1,
  });

  const referralsData = referralsResponse?.items || [];
  const total = referralsResponse?.total || 0;
    ...
    <div className="fade-in">
      ...
      >
        {referralsData.length === 0 ? (
          <Empty description={t('referrals.empty', 'Нет рефералов')} />
        ) : (
          <Table
            columns={columns}
