...
import { fileToBase64, type MediaInputValue } from '@/utils/media';
...
export const PartnersPage = () => {
  ...
  const handleEdit = (partner: Partner) => {
    ...
    form.setFieldsValue({
      ...
      logo: partner.logo_url
        ? ({ mode: 'url', url: partner.logo_url, fileList: [] } as MediaInputValue)
        : undefined,
      cover: partner.cover_image_url
        ? ({ mode: 'url', url: partner.cover_image_url, fileList: [] } as MediaInputValue)
        : undefined,
    });
  ...
  const resolveMedia = async (media?: MediaInputValue) => {
    if (!media) return undefined;
    if (media.mode === 'url') return media.url?.trim() || undefined;
    const file = media.fileList?.[0]?.originFileObj as File | undefined;
    if (file) {
      try {
  ...
  const handleSave = async (values: any) => {
    try {
      const logoUrl = await resolveMedia(values.logo);
      const coverUrl = await resolveMedia(values.cover);
      const payload = {
        ...values,
        logo_url: logoUrl ?? values.logo_url,
        cover_image_url: coverUrl ?? values.cover_image_url,
      };
    ...
    <div className="fade-in">
      ...
      >
        ...
        >
          ...
          <Form.Item name="logo" label={t('partners.logo', 'Логотип')} valuePropName="value">
            <MediaInput
          ...
          <Form.Item name="cover" label={t('partners.cover', 'Обложка')} valuePropName="value">
            <MediaInput
              placeholderUrl="https://example.com/cover.png"
              accept="image/*"
              label={undefined}
            />
          </Form.Item>
